...
void initializeFlagBoard(char fboard[5][5])
{
...
void printpboard(char playboard[5][5], char fboard[5][5])
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        for (int j = 0; j < 5; j++)
        {
            ...
            // If the player has placed a flag, display it
            if (fboard[i][j] == 'F')
            {
            ...
            // Otherwise display the player's board
            else
            {
...
void placeFlag(char fboard[5][5], char playboard[5][5], int row, int col, int &flagremaining)
{
    ...
    else if (playboard[row][col] != '#')
    {
 
        return;
    }
 
    else if (fboard[row][col] == 'F')
    {
 
        return;
    }
    else if (flagremaining > 0)
    {
        fboard[row][col] = 'F';
        flagremaining--;
    }
    else
    {
        ...
        // No flags left to place
        cout << "No flags remaining!" << endl;
    }
}
 
void removeFlag(char fboard[5][5], char playboard[5][5], int row, int col, int &flagremaining)
{
    if (validcell(row, col) == false)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            // No flag here to remove
            cout << "No flag placed here!" << endl;
            return;
        }
    }
}
...
void revealCell(char playerboard[5][5], int board[5][5], char fboard[5][5], int row, int col, int &revealedcells)
{
    ...
    // Base case 2:
    // If the cell has been flagged, stop recursion
    // This prevents the flood fill from overwriting a flag the player placed
    else if (fboard[row][col] == 'F')
    {
        ...
        for (int i = -1; i <= 1; i++)
        {
            ...
            for (int j = -1; j <= 1; j++)
            {
                ...
                if (validcell(neighbourRow, neighbourCol) == true)
                {
                    ...
                    revealCell(playerboard, board, fboard,
                               neighbourRow, neighbourCol, revealedcells);
                }
            }
        }
    }
...
bool playerMove(char playboard[5][5], int board[5][5], int &revealedcells, char fboard[5][5], int &flagremaining)
{
    int choice;
    int prow, pcol;
    ...
    if (choice == 1)
    {
        ...
        if (fboard[prow][pcol] == 'F')
        {
            cout << "Remove the flag first!" << endl;
            return true;
        }
        ...
        if (board[prow][pcol] == -1)
        {
            ...
            printpboard(playboard, fboard);
 
            return false;
        }
 
        revealCell(playboard, board, fboard, prow, pcol, revealedcells);
    }
    ...
    else if (choice == 2)
    {
        ...
        placeFlag(fboard, playboard, prow, pcol, flagremaining);
    }
    ...
    else if (choice == 3)
    {
        ...
        removeFlag(fboard, playboard, prow, pcol, flagremaining);
    }
...
int main()
{
    ...
    while (game == true)
    {
        ...
        // Show how many flags are left before printing the board
        cout << "Flags Remaining: " << flagremaining << endl;
        ...
        // Take player's move
        game = playerMove(playboard, board, revealedcells, fboard, flagremaining);
